...
import { HelmetContext } from "./HelmetProvider";
...
const DefaultHelmet: React.FC<HelmetProps> = (props: HelmetProps) => {
    const helmetContext = React.useContext(HelmetContext);

    if(helmetContext) {
        if(!props.title && helmetContext.title) props.title = helmetContext.title;
    }
